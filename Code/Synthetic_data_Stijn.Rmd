...
```{r, warning=FALSE}
synthesized = synthesize(data, method = cart, nsim = 10)
```
...
## 6a. Calculating the average parameters and statistics over the simulations

```{r}
average = apply(synthetic_analysis, 2, mean)
print(average)
```
...
```{r}
dataset_distribution(synthetic_analysis) #needs to be changed to boxplots and have the true data as reference
```
