...
for (i in seq(1, length(synthesized), 1)) { #looping through each simulation
  ...
  for (n in seq(1, imputation$m, 1)) { #looping through each imputation of the simulation
    ...
    true_data_index = sample(c(1:nrow(data)), size = (nrow(data)*0.5)) #sampling half of the index values of the original data
    true_data = data[true_data_index, 1:ncol(data)] #using the index values to get the rows of the corresponding indices
    true_data$Synth_or_true = 1 #setting the true data to 1
    synthetic_data_index = sample(c(1:nrow(syndata)), size = (nrow(syndata)*0.5)) #sampling half of the index values of the imputed data
    synthetic_data = syndata[synthetic_data_index, 1:ncol(syndata)] #using the index values to get the rows of the corresponding indices
    synthetic_data$Synth_or_true = 0 #setting the synthetic data to 0
    prediction_data = rbind(true_data, synthetic_data) #combining the two samples into a single data set
    prediction_model = glm(Synth_or_true~Pregnancies+Glucose+BloodPressure+SkinThickness+Insulin+BMI+DiabetesPedigreeFunction+Age+Outcome, data = prediction_data, family = "binomial") #setting up a prediction model that includes all variables
    result = predict_synth_or_true(prediction_data, prediction_model) #using the prediction function to get a data frame of true and predicted values
